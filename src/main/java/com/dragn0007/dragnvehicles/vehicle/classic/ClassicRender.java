...
public class ClassicRender extends EntityRenderer<Classic> {
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(new ResourceLocation(ValiantVehiclesMain.MODID, "classic"), "main");
    ...
    private final ClassicModel model;

    public ClassicRender(EntityRendererProvider.Context context) {
        super(context);
        this.model = new ClassicModel(context.bakeLayer(LAYER_LOCATION));
    }

    @Override
    public void render(Classic car, float rotation, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        poseStack.pushPose();
    ...
    @Override
    @NotNull
    public ResourceLocation getTextureLocation(Classic car) {
